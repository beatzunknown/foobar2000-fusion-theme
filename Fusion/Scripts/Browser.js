	...
	this.on_mouse = function (event, x, y) {
		...
		switch (event) {
			case "lbtn_down":
				...
				if (this.ishover && this.rowsCount == 0) {} else {
...
function seek_track(metadb) {
	if (g_browser) {
		...
		if (found) { // scroll to album and open showlist
			if (!properties.expandInPlace && scroll > (g_browser.rowsCount * g_browser.rowHeight) - wh) {} else {
				var scroll_new = Math.floor(a / g_browser.totalColumns) * g_browser.rowHeight;

				if (scroll_new <= scroll + 10 && scroll_new >= scroll - 10) {} else {
