...
if (ActiveX) {
	cInputbox.doc = new ActiveXObject("htmlfile");
}

oInputbox = function (w, h, default_text, empty_text, textcolor, backcolor, bordercolor, backselectioncolor, func, parentObjectName, bgeditcolor, txteditcolor) {
	this.font = playlistsFont;
	this.w = w;
	this.h = h;
	this.textcolor = textcolor;
	this.backcolor = backcolor;
	this.bordercolor = bordercolor;
	this.bgeditcolor = bgeditcolor;
	this.txteditcolor = txteditcolor;
	...
	this.draw = function (gr, x, y) {
		...
		if (this.edit || this.text.length > 0)
			gr.FillSolidRect(x - 1, y + 1, (this.w + 2), this.h - 2, this.bgeditcolor);
		else
		...
		if (this.text.length > 0) {
			gr.GdiDrawText(this.text.substr(this.offset), this.edit ? this.font : this.font, this.txteditcolor, this.x, this.y, this.w, this.h, DT);
		} else {
			gr.GdiDrawText(this.empty_text, this.font, textcolor, this.x, this.y, this.w, this.h, DT);
		}
		// draw cursor
	...
	this.drawcursor = function (gr) {
		if (cInputbox.cursor_state) {
			if (this.Cpos >= this.offset) {
				...
				var lt = 1;
				gr.DrawLine(x1, y1, x2, y2, lt, this.txteditcolor);
			}
		}
	}
	...
	this.show_context_menu = function (x, y) {
		...
		cInputbox.clipboard = ActiveX ? cInputbox.doc.parentWindow.clipboardData.getData("Text") : "";
		...
		switch (idx) {
			case 1:
				if (this.edit && this.select) {
					ActiveX ? cInputbox.doc.parentWindow.clipboardData.setData("Text", this.text_selected) : false;
				}
				break;
			case 2:
				if (this.edit && this.select) {
