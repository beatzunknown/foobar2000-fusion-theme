		...
		if (toolbar.buttons.length) {
			if (largeNowPlayingArtwork && pl_w > 100 && wh > 750 && properties.showleftPanel) {
				...
				if (fb.IsPlaying || fb.IsPaused) {
					...
					gr.GdiDrawText(g_title, barTextFont, col_bar_title, largeNowPlayingArtwork_TitleArtistX + 0, cover_y + 8, pl_deafult_w - 30, 25, DT_LEFT | DT_CALCRECT | DT_TOP | DT_END_ELLIPSIS | DT_NOPREFIX);
					gr.GdiDrawText(g_artist, barTextFont2, col_bar_artist, largeNowPlayingArtwork_TitleArtistX + 0, cover_y + 8 + gr.CalcTextHeight(g_title, barTextFont) + 3, pl_deafult_w - 30, 25, DT_LEFT | DT_CALCRECT | DT_TOP | DT_END_ELLIPSIS | DT_NOPREFIX);
...
function on_mouse_wheel(step) {
	...
	if (g_scrollbar && !g_dragA) {
		if (utils.IsKeyPressed(VK_SHIFT)) {} else {
